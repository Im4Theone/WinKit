...
public sealed partial class SettingsViewModel : ObservableObject, IAsyncInitializable
{
    ...
    [RelayCommand]
    private async Task RunAsAdministratorAsync()
    {
        var confirmed = _dialogService.Confirm(new ConfirmationRequest
        {
        ...
        // RelaunchElevated's ShellExecuteEx("runas") blocks until the UAC prompt is
        // resolved; run it off the UI thread so WinKit doesn't appear frozen while it's up.
        var elevated = await Task.Run(() => _elevationService.RelaunchElevated(string.Empty));
