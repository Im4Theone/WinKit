...
public abstract partial class ToolPageViewModelBase : ObservableObject
{
    ...
    /// <summary>Deep-links into the tab hosting the given view model type, e.g. from Diagnostics' "Review" actions.</summary>
    public void SelectTab<TViewModel>() where TViewModel : ObservableObject
    {
        var tab = Tabs.FirstOrDefault(t => t.ViewModel is TViewModel);
        if (tab is not null)
        {
            SelectedTab = tab;
        }
